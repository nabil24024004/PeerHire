  ...
  useEffect(() => {
    const fetchPayments = async () => {
      ...
      try {
        ...
        // Calculate stats
        const totalEarned = paymentsData?.filter((p) => p.status === "completed").reduce((sum, p) => sum + Number(p.amount), 0) || 0;
        const completedJobs = paymentsData?.filter((p) => p.status === "completed").length || 0;
        const outstandingPayments = paymentsData?.filter((p) => p.status === "pending").length || 0;

        setStats({ totalEarned, completedJobs, outstandingPayments });
      } catch (error: any) {
        toast({
          title: "Error",
