...
const FreelancerBrowseJobs = () => {
  ...
  const fetchJobs = async (userId: string) => {
    try {
      ...
      const { data: jobsData, error: jobsError } = await supabase
        ...
        .select(`
          *,
          profiles!jobs_hirer_id_fkey(full_name, department)
        `)
        .eq('status', 'open')
      ...
      const savedIds = new Set(savedData?.map(sj => sj.job_id) || []);
      setSavedJobIds(savedIds);

      // Fetch complete saved jobs data
        ...
        <div className="flex flex-col gap-3 md:gap-4">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              {job.subject && (
                <Badge variant="secondary" className="text-xs">{job.subject}</Badge>
              )}
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-4 pt-3 md:pt-4 border-t border-border">
          ...
          <div className="flex items-center gap-2 text-xs md:text-sm">
            ...
            <div className="min-w-0">
              ...
              <p className="font-semibold">{job.page_count}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 text-xs md:text-sm">
  ...
  const handleMessageHirer = async (job: Job) => {
    try {
      ...
      navigate(`/freelancer/messages?conversation=${conversationId}`);
    } catch (error: any) {
      toast({
