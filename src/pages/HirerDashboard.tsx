...
const HirerDashboard = () => {
  ...
  const fetchDashboardData = async (userId: string) => {
    try {
      ...
      const jobsWithCounts: JobWithApplications[] = (jobs || []).map(job => ({
        ...job,
      ...
      // Calculate stats
      const openCount = jobsWithCounts.filter(j => j.status === 'open' || j.status === 'in_progress').length;
      const completedCount = jobsWithCounts.filter(j => j.status === 'completed').length;
      const totalSpent = jobsWithCounts
